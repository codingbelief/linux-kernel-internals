...
tRFC 包含两个部分的时间，一是完成刷新操作所需要的时间，由于 DRAM Refresh 是同时对所有 Bank 进行的，刷新操作会比单个 Row 的 Active + Precharge 操作需要更长的时间；tRFC 的另一部分时间为了降低平均功耗而引入的延时，DRAM Refresh 操作所消耗的电流也会比单个 Row 的 Active + Precharge 操作要大的多，tRFC 限制 Refresh 操作的频率。
