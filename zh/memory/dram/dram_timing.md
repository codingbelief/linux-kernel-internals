...
由于 DRAM 的特性，Row 中的数据在被读取到 Sense Amplifiers 后，需要进行 Restore 的操作（细节请参考 [DRAM Storage Cell](./dram_storage_cell.html) 章节）。Restore 操作可以和数据的读写同时进行，即在这个阶段，Controller 可能发送了 Read 或者 Write Command 进行数据读写。  
