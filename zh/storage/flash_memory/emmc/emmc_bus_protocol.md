...
如果 Host 发送的是 Single Block Read 的 Command，那么 eMMC Device 只会发送一个 Block 的数据（一个 Block 的数据的字节数由 Host 指定或者 eMMC Device 设定的默认值，更多细节请参考 [eMMC 工作模式](/emmc_modes.html) 章节）。
