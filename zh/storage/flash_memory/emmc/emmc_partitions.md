...
eMMC 中定义了 Boot State，在 Power-up、HW reset 或者 SW reset 后，如果满足一定的条件，eMMC 就会进入 Boot State。进入 Boot operation mode 的条件如下：

1. CMD 信号保持低电平超过 74 个时钟周期，会触发 Normal boot operation，进入 Boot State。
