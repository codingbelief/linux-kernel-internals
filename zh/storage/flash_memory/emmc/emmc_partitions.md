...
一般情况下，Boot Area Partitions 和 RPMB Partition 的容量大小是由芯片厂家定义的，用户不可修改，通常都为 4MB。General Purpose Partitions (GPP) 在出厂时默认的容量大小都为 0，需要用户主动配置所要使用的 GPP 的容量大小，各个 GPP 的容量大小可以不一样。
