...
在某些 eMMC 中，Flash Controller 还会完成一些特殊的工作。例如，一个 16GB 的 TLC eMMC，在出厂后， 先让内部的 NAND Flash 处于 SLC 状态，这样可以在初期获得较好的性能。当达到一定条件后（eg. 使用了总存储单元的 1/3 时），Flash Controller 将 NAND Flash 切换到 TLC 状态，保证数据存储容量。  
